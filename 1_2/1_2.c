...
double ex_lim (double eps){
    double n = 1;
    double result_1 = pow(1.0 + 1.0/(n), n);
    double result_2 = pow(1.0 + 1.0/(n+1.0), n+1.0);
    while (fabs (result_2 - result_1) > eps){
        result_1 = result_2;
        n++;
        result_2 = pow(1.0 + 1.0/(n+1.0), n+1.0);
    }
    return result_1;
}
...
double gamma_lim (double eps){
    ...
    do{
        ...
        for (k; k <= m; ++k){
            fact *= k;
            element = (C(m,k)*(pow(-1, k)/k)*log(fact));
            sum += element;
        }
        result_2 = sum;
        m++;
...
double gamma_row(double eps){
    ...
    do {
        sum_1 = sum_2;
        element = (1/(pow(sqrt(k), 2))) - (1/k); 
        sum_2 += element;
        if (element < epsilon)
            sum_1 = 0.0;
        k++;
    } while (fabs(sum_2 - sum_1) > eps);
...
double gamma_dich (double eps){
    double a = 0.0;
    double b = 1.0;
    double prex = b; 
    double x = (a+b)/2.;
    while (fabs(prex - x) > eps){
        if (((exp(-x) - gam_lim(x)) * (exp(-a) - gam_lim(a))) > 0) {
            a = x;
        }
        else { b = x; }
        prex = x;
...
int main (int argc, char *argv[]){
    ...
    if (argc != 2){
        printf("Необходимо ввести один аргумент: количество знаков после запятой");
    }
    int l = atoi(argv[1]);
    double eps = pow(10, -l);
